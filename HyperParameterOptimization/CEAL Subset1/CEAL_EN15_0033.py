...
class IterationManager():
    def __init__(self, maximages, startamount, iterationamount, initial_treshold =  0.15, initial_decayrate = 0.0033) -> None:
        self.image_list = []
...
def create_model_and_train(iterator, outputdir, queue):
    ...
    DatasetCatalog.register("supermarket_train", lambda: get_combined_dict(["../supermarket_dataset/alteredannotations/D2S_training_newsplit.json", "../supermarket_dataset/alteredannotations/D2S_validation_newsplit.json"], iterator.labeled_image_list, pseudolabelurl, pseudoimagenumbers))
...
if __name__ == '__main__':
    ...
    EXPERIMENTNAME = "ceal0.15_0.0033"
    ...
    while iterator.keep_iterating:
        ...
        p.start()
        entropylist = queue.get()
        p.join()

        print(f"Entropy List sorted {entropylist}")
        
        iterator.cealiteration(entropylist, entropylist) # transfer XX images from Du to Dl through random choice (RANDOM)
