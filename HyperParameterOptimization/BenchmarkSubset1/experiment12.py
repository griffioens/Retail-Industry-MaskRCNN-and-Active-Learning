...
def get_combined_dict(listing):
    final_list = []
    for dataset in listing:
        final_list = final_list + get_supermarket_dicts(dataset)
    return final_list


if __name__ == '__main__':
    # Calculating the number of images in the training set to determine epoch length
    image_count = len(get_combined_dict(["../supermarket_dataset/annotations/annotations/D2S_training.json", "../supermarket_dataset/annotations/annotations/D2S_augmented.json"]))
    print(f"Length of training set: {image_count}")

    # CONSTANTS
    EXPERIMENTNAME = "experiment12"
    EPOCHS = 10
    LEARNING_RATE = 0.001
    step_1 = int((image_count/2) * EPOCHS * 0.70)
    step_2 = int((image_count/2) * EPOCHS * 0.90)
    LEARNING_RATE_DECAY_STEP = (step_1, step_2,) #empty list if no decay, otherwise an iteratble with step numberw here to decay weight eg. (3000, ) will decay the weight at the 3000th iteration (with 2 images per iteration this is at image 6000)
    ...
    DatasetCatalog.register("supermarket_train", lambda: get_combined_dict(["../supermarket_dataset/annotations/annotations/D2S_training.json", "../supermarket_dataset/annotations/annotations/D2S_augmented.json"]))
    ...
    DatasetCatalog.register("supermarket_validation", lambda: get_supermarket_dicts("../supermarket_dataset/alteredannotations/D2S_test_newsplit.json"))
