...
def create_model_and_train(iterator, outputdir, queue):
    ...
    DatasetCatalog.register("supermarket_train", lambda: get_combined_dict(["../supermarket_dataset/alteredannotations/D2S_training_newsplit.json", "../supermarket_dataset/alteredannotations/D2S_validation_newsplit.json"], iterator.labeled_image_list))
    ...
    DatasetCatalog.register("supermarket_unlabled", lambda: get_combined_dict(["../supermarket_dataset/alteredannotations/D2S_training_newsplit.json", "../supermarket_dataset/alteredannotations/D2S_validation_newsplit.json"], iterator.unlabeled_image_list))
    list_classes = class_list("../supermarket_dataset/alteredannotations/D2S_training_newsplit.json")
    ...
    print("before empty cache", flush=True)
    torch.cuda.empty_cache()
    print("before entropy calculation", flush=True)
    # Calculating least-confident and sorting, then sending outputs from child to parent thread
    outputs = entropycalc(alloutputs)
    print("entropy calculations complete, putting data in queue", flush=True)
    queue.put(outputs)
    print("queue put complete", flush=True)
    torch.cuda.empty_cache()
...
if __name__ == '__main__':
    ...
    iterator.loadcocolike(["../supermarket_dataset/alteredannotations/D2S_training_newsplit.json", "../supermarket_dataset/alteredannotations/D2S_validation_newsplit.json"]) # load hte imagenumbers of the 2 datasets into memory
    ...
    while iterator.keep_iterating:
        ...
        p.start()
        leastconfidentlist = queue.get()
        p.join()

        print(f"list of leastconfident sorted {leastconfidentlist}")
        
        iterator.leastconfidentiteration(leastconfidentlist) # transfer 100 images from Du to Dl through random choice (RANDOM)
